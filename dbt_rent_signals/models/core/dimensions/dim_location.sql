{{
  config(
    materialized='table',
    description='SCD Type 2 location dimension with historical tracking of geographic changes'
  )
}}

with location_snapshot as (
    select * from {{ ref('snap_location') }}
),

dimension_table as (
    select
        -- Surrogate key (auto-generated by snapshot)
        {{ dbt_utils.generate_surrogate_key(['location_business_key', 'dbt_valid_from']) }} as location_key,
