{{
  config(
    materialized='table',
    description='SCD Type 2 economic series dimension with historical tracking of series metadata changes'
  )
}}

with series_snapshot as (
    select * from {{ ref('snap_economic_series') }}
),

dimension_table as (
    select
        -- Surrogate key
        {{ dbt_utils.generate_surrogate_key(['series_business_key', 'dbt_valid_from']) }} as series_key,
