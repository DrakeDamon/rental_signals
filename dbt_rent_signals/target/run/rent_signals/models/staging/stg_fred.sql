  ...
  create or replace   view RENTS.DBT_DEV_staging.stg_fred
...
with source_data as (
    select * from RENTS.RAW.fred_cpi_long
),
...
cleaned as (
    select
        ...
        md5(cast(coalesce(cast(label as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(category as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(seasonal_adjustment as TEXT), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(frequency as TEXT), '_dbt_utils_surrogate_key_null_') as TEXT)) as series_content_hash,
